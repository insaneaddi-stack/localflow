...
if [ -x "$CANDIDATE" ]; then PY="$CANDIDATE"; else PY="$REAL"; fi   # standalone (uv) : le binaire est déjà le vrai
...
. ./env.sh
if ! "$APP/Contents/MacOS/LocalFlow" -c "import sys; import localflow" 2>/tmp/localflow-build-test.log; then
